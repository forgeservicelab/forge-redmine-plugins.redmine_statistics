class StatsController < ApplicationController
  ...
  def index
    ...
    case adapter_type
      when :postgresql
        @min_closure = Issue.connection.select_all("SET intervalstyle TO postgres_verbose; SELECT JUSTIFY_HOURS(MIN(issues.updated_on - issues.created_on)) AS duration FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.status_id = 5 AND issues.updated_on != issues.created_on AND issues.subject != 'Welcome to FORGE Service Lab!' AND projects.identifier = '#{params[:project_id]}'").to_a
        @max_closure = Issue.connection.select_all("SET intervalstyle TO postgres_verbose; SELECT JUSTIFY_HOURS(MAX(issues.updated_on - issues.created_on)) AS duration FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.status_id = 5 AND issues.updated_on != issues.created_on AND issues.subject != 'Welcome to FORGE Service Lab!' AND projects.identifier = '#{params[:project_id]}'").to_a
        @avg_closure = Issue.connection.select_all("SET intervalstyle TO postgres_verbose; SELECT JUSTIFY_HOURS(AVG(issues.updated_on - issues.created_on)) AS duration FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.status_id = 5 AND issues.updated_on != issues.created_on AND issues.subject != 'Welcome to FORGE Service Lab!' AND projects.identifier = '#{params[:project_id]}'").to_a
      when :mysql || :mysql2
